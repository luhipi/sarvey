...
lint: ## check style with flake8
	flake8 --max-line-length=120 sarvey tests > ./tests/linting/flake8.log || \
		(cat ./tests/linting/flake8.log && exit 1)
	pycodestyle sarvey --exclude="*.ipynb,*.ipynb*" --max-line-length=120 > ./tests/linting/pycodestyle.log || \
		(cat ./tests/linting/pycodestyle.log && exit 1)
	pydocstyle sarvey > ./tests/linting/pydocstyle.log || \
...
coverage: ## check code coverage quickly with the default Python
	coverage erase
	coverage run --source sarvey -m pytest
...
gitlab_CI_docker:  ## Build a docker image for CI use within gitlab
	cd ./tests/CI_docker/; bash ./build_sarvey_testsuite_image.sh
